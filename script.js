...
function addClasses() {
  ...
  const lista = elementSelected.parentNode.parentNode;
...
function removeElements() {
  ...
  elementToRemove.forEach(el => firstList.removeChild(el));
}
...
function generateList() {
  ...
  firstListChilds.forEach(el => {
    const elChildren = el.firstChild;
    const item = document.createElement("li");
    const button = document.createElement("button");
    const text = document.createTextNode(elChildren.textContent);
    button.className = elChildren.className;
...
function addDisabled() {
  const finalButton = document.querySelector("#list2>li:last-of-type>button");
